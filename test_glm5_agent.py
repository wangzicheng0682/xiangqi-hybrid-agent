...
api_key = os.getenv('ALIYUN_API_KEY')  # 百炼用同一个key
FEN = 'rnbakabnr/9/1c5c1/p1p1p1p1p/9/6P2/P1P1P3P/1C5C1/9/RNBAKABNR w - - 0 3'

print('【系统启动】')
print(f'模型: GLM-5 (enable_thinking=True)')
print(f'局面: {FEN}')
print()
...
resp = requests.post(
    ...
    json={
        ...
        'messages': [
            ...
            {'role': 'user', 'content': '你打算怎么分析这个局面？列出想调用的工具、目的。（只是规划，不执行）'}
        ],
        'temperature': 0.7,
        'max_tokens': 800,
...
for round_num in range(3):
    ...
    if tool_calls_buffer:
        tool_names = [v['name'] for v in tool_calls_buffer.values() if v['name']]
        print(f'[GLM-5决定调用] {tool_names}')

        assistant_msg = {'role': 'assistant', 'content': current_content, 'tool_calls': []}
