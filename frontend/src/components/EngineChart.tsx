...
export const EngineChart: React.FC<EngineChartProps> = ({ compact = false, fluid = false }) => {
  ...
  // 流体尺寸：从容器测量
  const [measured, setMeasured] = useState({ width: 500, height: 250 });

  // 固定尺寸
  const fixedWidth = compact ? 480 : 400;
  const fixedHeight = compact ? 80 : 120;

  // 最终尺寸：流体取测量值，否则用固定值
  const width = fluid ? measured.width : fixedWidth;
  const height = fluid ? measured.height : fixedHeight;
  ...
  const data = evalHistory.slice(-20);
  ...
  const scoreColor = currentScore === 0 ? '#888' : currentScore > 0 ? '#ef4444' : '#3b82f6';
  ...
  if (data.length === 0) {
    return (
      <div ref={containerRef} style={{ ...styles.container, width: fluid ? '100%' : width, height }}>
        <div style={styles.header}>
          <span style={styles.icon}>📈</span>
          <span style={styles.title}>引擎评分</span>
        </div>
        <div style={{ ...styles.emptyChart, height: fluid ? '100%' : height - 40 }}>
  ...
  return (
    <div ref={containerRef} style={{ ...styles.container, width: '100%', height: '100%' }}>
      <div style={styles.header}>
        <span style={styles.icon}>📈</span>
        <span style={styles.title}>引擎评分</span>
